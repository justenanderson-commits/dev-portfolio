...
test('about page', async ({ page }) => {
  ...
  const educationTitle = await page
    .getByRole('heading', {
      name: 'Turing School of Software and Design - Denver, Colorado',
    })
    .first()
  const experienceTitle = await page.getByRole('heading', {
    name: 'Multiple Schools in Colorado & Costa Rica',
  })
  const certsAndAwardsTitle = await page.getByRole('heading', {
    name: 'Front-End Engineering Certificate',
  })
...
test('test', async ({ page }) => {
  ...
  const resumeTitle = await page1
    .getByText('Justen Anderson PDF Resume.pdf')
    .nth(1)
